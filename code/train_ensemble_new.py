...
def main():
    ...
    if TRAINING:
        ...
        try:
            ...
            ensemble.train_meta_model(base_val_preds, y_val)
            ensemble.optimize_weights(base_val_preds,   y_val, n_trials=100)
            # Save the ensemble model
            val_preds = ensemble.predict(base_val_preds)
            print("Val-RMSE after ensemble:", 
        ...
        try:    # Calculate metrics for each department
            ...
            for department_idx, department_name in DEP_NAMES.items():
                ...
                results.append({
                    ...
                    'NRMSE 0-19 Training (same-as val)': nrmse_dengrate_019_val,  # Using validation metrics for both
                    'NRMSE All Training (same-as val)': nrmse_dengrate_all_val,
